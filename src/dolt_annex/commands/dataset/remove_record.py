...
class RemoveRecord(cli.Application):
    """Remove a single record matching a filter from a dataset. Does not remove files from the filestore."""
    ...
    async def main(self, *args) -> int:
        ...
        async with (
            ...
            rows = iter(table.get_rows(filters=self.filters))
            row = next(rows, None)
            if next(rows, None):
                raise ValueError("Filter matches more than one row; refusing to remove multiple rows.")
            if not row:
                raise ValueError("Filter does not match any rows; nothing to remove.")
            await table.remove(row)
            print(f"Removed row {row} from table '{self.table_name}' in dataset '{self.dataset}'")
        return 0
