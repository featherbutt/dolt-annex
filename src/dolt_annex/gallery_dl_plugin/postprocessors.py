...
def gallery_dl_post(metadata: dict):
    ...
    for table_row in source.post_metadata(metadata):
        ...
        import_bytes(tasks, local_uuid, ContentAddressableStorage.from_local(dolt_annex_config).file_store, table, table_row, metadata_bytes, "json")
...
def gallery_dl_import(source: GalleryDLSource, metadata: dict):
    ...
    import_file(tasks, local_uuid, local_file_store, submissions_table, source.table_key(metadata), temp_path, metadata["extension"], metadata["sha256"])
    for metadata_key in source.file_metadata(metadata):
        import_file(tasks, local_uuid, local_file_store, metadata_table, metadata_key, temp_path.parent / (temp_path.name + ".json"), "json")

def import_file(tasks: asyncio.TaskGroup, local_uuid: UUID, filestore: FileStore, file_table: FileTable, table_key: TableRow, from_path: Path, extension: str, sha256: Optional[str] = None):
...
def import_bytes(tasks: asyncio.TaskGroup, local_uuid: UUID, local_filestore: FileStore, file_table: FileTable, table_key: TableRow, file_bytes: bytes, extension: str, sha256: Optional[str] = None):
    """Import a file into the dolt-annex dataset, and add a corresponding row to given table with the given table key."""
    if not sha256:
