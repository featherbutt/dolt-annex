...
@pytest_asyncio.fixture(params=all_filestore_type_parameters(pathlib.Path("from")))
async def from_repo_and_keys(request, temp_dir, base_config):
...
@pytest_asyncio.fixture(params=all_filestore_type_parameters(pathlib.Path("to")))
async def to_repo(request, temp_dir, base_config):
    filestore_model: FileStoreModel = request.param
    async with filestore_model.open(base_config) as filestore:
        to_uuid = uuid.uuid4()
        repo = Repo(
...
async def test_async_move(setup: EnvironmentForTest, temp_dir: pathlib.Path, from_repo_and_keys: tuple[Repo, list[Sha256e]], to_repo: Repo):
    from_repo, file_keys = from_repo_and_keys
