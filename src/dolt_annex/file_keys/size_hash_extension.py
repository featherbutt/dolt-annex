...
class SizeHashExtensionFileKey(FileKey, is_abstract=True):
    """
    An abstract parent class for FileKeys that uses the format: <preifx>-s<size>--<hash>[.<extension>]
    """
    ...
    def make(cls, size: int, hash_string: str, extension: Optional[str] = None) -> Self:
        ...
        key=f"{cls.prefix}-s{size}--{hash_string}".encode('utf-8')
        if extension:
...
class Generator[T: SizeHashExtensionFileKey](FileKeyGenerator):
...
class Sha256E(SizeHashExtensionFileKey, prefix="SHA256E", lower_extensions=False):
    """
...
class MD5e(SizeHashExtensionFileKey, prefix="MD5e", lower_extensions=True):
    """
...
class SHA1e(SizeHashExtensionFileKey, prefix="SHA1e", lower_extensions=True):
    """
