...
- Files with different content **must** be assigned different names.
...
There are currently two local filestore implementation: AnnexFS and ArchiveFS.

AnnexFS is a simple filesystem based store called AnnexFS, modeled after git-annex's file layout. Files are stored in a nested directory structure with two levels of indirection based on the first six characters of the file key's md5 hash.
...
ArchiveFS works by grouping imported files together in TAR archives, reducing the total number of files needed. Once these TAR files reach a configurable max size, they are "frozen" and can be safely backed up with tools like rsync.

Finally, remote repos are represented locally using the SFTP filestore type.
