...
class DownloadBatch(cli.Application):
    ...
    def main(self, *args):
        ...
        with Downloader(self.parent.config, self.batch_size) as downloader:
            while True:
                urls, num_urls = sql.random_batch(self.url_prefix, downloader.dolt.cursor, self.batch_size)
    ...
    def download_file(self, downloader: AnnexCache, url: str) -> Optional[str]:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as temp_file:
            try:
                ...
                abs_path = Path(os.path.abspath(temp_file.name))
                key = key_from_file(abs_path)
                move_files(shutil.move, {AnnexKey(key): Path(abs_path)})  
